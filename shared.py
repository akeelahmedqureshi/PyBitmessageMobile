...
def checkSensitiveFilePermissions(filename):
    if sys.platform == 'win32':
        ...
        try:
            ...
            print('Could not determine filesystem type. %s', filename)
        present_permissions = os.stat(filename)[0]
        disallowed_permissions = stat.S_IRWXG | stat.S_IRWXO
        return present_permissions & disallowed_permissions == 0
...
def fixSensitiveFilePermissions(filename, hasEnabledKeys):
    ...
    try:
        ...
        print('Keyfile permissions automatically fixed.')

    except Exception:
        print('Keyfile permissions could not be fixed.')
        raise
...
def checkAndShareObjectWithPeers(data):
    ...
    if not protocol.isProofOfWorkSufficient(data):
        print('Proof of work is insufficient.')
        return 0
...
def _checkAndShareUndefinedObjectWithPeers(data):
    ...
    print('advertising inv with hash: %s', hexlify(inventoryHash))
...
def _checkAndSharePubkeyWithPeers(data):
    ...
    if addressVersion >= 4:
        tag = data[readPosition:readPosition + 32]
        print('tag in received pubkey is: %s', hexlify(tag))
    else:
        tag = ''
